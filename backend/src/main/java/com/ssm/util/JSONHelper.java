...
public class JSONHelper {
    ...
    //MOdify by WLing 2016-11-26 如果属性的值为空则不处理
	/*// 将JSON转换成POJO,其中beanClz为POJO的Class
	public static Object json2Object(String json, Class<?> beanClz) {
		return JSONObject.toBean(JSONObject.fromObject(json), beanClz);
	}*/
    // 将JSON转换成POJO,其中beanClz为POJO的Class
    public static Object json2Object(String json, Class<?> beanClz) {//json字符串的写法以{}表示一个对象，字符串必须加引号,null必须替换为"",数组为[]
        //JSONObject jsonObj1 = JSONObject.fromObject(json);
        JsonConfig config = new JsonConfig();
        config.setRootClass(beanClz);
        config.setJsonPropertyFilter(new PropertyFilter() {
        	@Override
            public boolean apply(Object source, String name, Object value) {//source instanceof Person && name.equals("address")
        ...
        JSONObject jsonObject = (JSONObject) JSONSerializer.toJSON(json, config);
        return JSONObject.toBean(jsonObject, config);
    }
    // 将JSON转换成POJO,其中beanClz为POJO的Class
    public static Object json2ObjectUpCharacter(String json, Class<?> beanClz) {
        JSONObject jsonObj1 = JSONObject.fromObject(json);
        JsonConfig config = new JsonConfig();
        ...
        config.setRootClass(beanClz);
        return JSONObject.toBean(jsonObj1, config);
    }
    /**
     * json转换为java对象
     *
    ...
    public static Map<String, String> parseRequestJson(String json){
        ...
        try {
            ...
            logger.error("解析传入的json出现未知错误{}", e.getMessage());
            return null;
        }
        return result;
    }
